...
# Tests connectivity to various endpoints and services
...
# Function to check endpoint with curl or wget
check_endpoint() {
    local name="$1"
    local url="$2"
    ...
    if command_exists curl; then
        status_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time "$TIMEOUT" "$url" 2>/dev/null) || error_msg="Connection failed or timed out"
    elif command_exists wget; then
...
finalize_report() {
    ...
    # Show the full report if verbose mode is enabled
    if [[ "$VERBOSE" == "true" && -z "$REPORT_FILE" ]]; then
        echo ""
        echo "Full Report:"
...
if [[ -f "$ENV_FILE" ]]; then
    source "$ENV_FILE"
    [[ "$VERBOSE" == "true" ]] && log "Loaded environment configuration from $ENV_FILE" "INFO"
fi
...
if [[ "$REGION" == "primary" ]]; then
    ...
    DB_PORT="${PRIMARY_DB_PORT:-5432}"
    REDIS_HOST="${PRIMARY_REDIS_HOST:-primary-redis.internal}"
    REDIS_PORT="${PRIMARY_REDIS_PORT:-6379}"
else
...
if [[ ${#EXTERNAL_ENDPOINTS[@]} -eq 0 ]]; then
    # Default external endpoints to check
    EXTERNAL_ENDPOINTS=(
...
# Start the report
echo "Network Connectivity Check - $(date)" > "$TEMP_REPORT_FILE"
echo "Environment: $ENVIRONMENT, Region: $REGION" >> "$TEMP_REPORT_FILE"
echo "" >> "$TEMP_REPORT_FILE"
